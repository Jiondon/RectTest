...
VisionEllipseItem_New::VisionEllipseItem_New(bool rotation, VisionItem *parent):VisionItem(parent)
{
...
void VisionEllipseItem_New::setRect(qreal x, qreal y, qreal width, qreal height)
{
    m_width = width;
    m_height = height;
    m_x = x;
...
QPainterPath VisionEllipseItem_New::getPainterPath()
{
    QPainterPath path;
...
bool VisionEllipseItem_New::getPosInArea(qreal x, qreal y)
{
...
void VisionEllipseItem_New::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
    Q_UNUSED(widget)
    Q_UNUSED(option)
...
QRectF VisionEllipseItem_New::boundingRect() const
{
...
void VisionEllipseItem_New::mousePressEvent(QGraphicsSceneMouseEvent *event)
{
    qDebug() << "aaa";
...
void VisionEllipseItem_New::mouseReleaseEvent(QGraphicsSceneMouseEvent *event)
{
    qreal qc = sqrt(fabs((m_width/2)*(m_width/2) - (m_height/2)*(m_height/2)));
    //两个焦点需要平移到item的中心点
    qreal dx1 = event->pos().x()-(qc*cos(m_angle*pi/180)+m_width/2);
...
void VisionEllipseItem_New::mouseMoveEvent(QGraphicsSceneMouseEvent *event)
{
    qDebug()<<directCursor;
    if(directCursor != arrowsUp){
        if(!m_bEdit){
...
void VisionEllipseItem_New::hoverMoveEvent(QGraphicsSceneHoverEvent *event)
{
    QGraphicsItem::hoverMoveEvent(event);
}

void VisionEllipseItem_New::initItem()
{
    m_lstRect.clear();
...
void VisionEllipseItem_New::updateItem()
{
...
void VisionEllipseItem_New::updatePointF()
{
...
void VisionEllipseItem_New::updateBoundingRect()
{
    //已经在旋转后进行了换算
    //获取中心点，两条对角线的交点，
...
void VisionEllipseItem_New::slotIndex(int index)
{
...
void VisionEllipseItem_New::slotArrowsItem()
{
    directCursor = arrowsUp;
    m_bCircle = true;
}

void VisionEllipseItem_New::slotArrowsItem_leave()
{
    directCursor = normal_rect;
    m_bCircle = false;
}
